...
function RingingOverlay({
  ringingUser,
  handleJoinRoomAcceptClick,
  handleJoinRoomDeclineClick,
}) {
  const [showOverlay, setShowOverlay] = React.useState(true);

  React.useEffect(() => {
    setShowOverlay(!!ringingUser);
  }, [ringingUser]);
  ...
  function handleEscape(e) {
    if (e.key === 'Esc' || e.key === 'Escape') {
      ...
      handleJoinRoomDeclineClick({ id: ringingUser.id });
    }
  }

  function acceptCall(e) {
    if (!ringingUser || !ringingUser.id) return;
    handleJoinRoomAcceptClick({ id: ringingUser.id });
  }
      ...
      {showOverlay ? (
        <div className="fixed inset-0 bg-gray-900 bg-opacity-95">
          <div className="w-full h-full flex flex-col justify-center items-center">
            ...
            <div className="text-3xl sm:text-4xl text-white mt-2">
              {username}
            </div>
...
RingingOverlay.propTypes = {
  ...
  handleJoinRoomAcceptClick: PropTypes.func.isRequired,
  handleJoinRoomDeclineClick: PropTypes.func.isRequired,
};
